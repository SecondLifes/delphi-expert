...
TObjectManager. It assumes you have already connected to the database and configured the
mapping (see the TMS Aurelius documentation's database-connection and mapping chapters).
Querying objects using criteria and projections is covered in the TMS Aurelius
documentation's Queries chapter.
...
Create a TObjectManager by passing an IDBConnection instance:
...
To use a mapping model other than the default, pass a
...
TAureliusConnection component, and start working without writing construction code.
...
Refer to the TMS Aurelius documentation's Queries chapter for filtering, ordering,
