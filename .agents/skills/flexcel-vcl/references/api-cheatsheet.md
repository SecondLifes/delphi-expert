...
Dates: write a `TDateTime` by converting with the workbook's base date:

```pascal
serial := TFlxDateTime.DateTimeToSerial(dt, xls.OptionsDates1904);
xls.SetCellValue(r, c, serial, dateXF);
```

(Object Pascal has no `?:` ternary operator — pass `xls.OptionsDates1904` straight through as the boolean argument.) Simpler: use `TFlxNumberFormat` to build the format and write the `TDateTime` directly via the number APIs in newer versions — in practice APIMate will generate the correct call for you.
