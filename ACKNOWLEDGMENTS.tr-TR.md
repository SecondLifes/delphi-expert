...
| [ACBr Project](https://github.com/frones/ACBr) (resmi [Projeto ACBr](https://projetoacbr.com.br)'ın topluluk aynası) | Brezilya ticari otomasyon bileşenleri (NFe, CTe, Boleto, TEF, mali yazıcılar) — `.agents/rules/acbr-patterns.md` ve `acbr-components` skill'i `TACBrNFe`/`TACBrCTe` vb. etrafındaki service-wrapper izolasyonu ve yaşam döngüsü kurallarını öğretir | LGPL-2.1 |
...
| [TMS FlexCel](https://www.tmssoftware.com/site/flexcel.asp) (TMS Software) | Excel/PDF/HTML üretim kütüphanesi — `flexcel-vcl` ve `flexcel-net` skill'leri sırasıyla Delphi ve .NET için `TXlsFile`/`XlsFile` ve `TFlexCelReport`/`FlexCelReport` API'lerini öğretir | Ticari lisans gerekir; çalışma zamanında Office/Excel kurulumu gerekmez |
